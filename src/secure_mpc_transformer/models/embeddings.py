...
class SecureEmbeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, token_type_ids: torch.Tensor | None = None,
                position_ids: torch.Tensor | None = None) -> SecureValue:
...
class SecurePositionalEncoding(nn.Module):
    ...
    def forward(self, x: SecureValue, position_ids: torch.Tensor | None = None) -> SecureValue:
...
class SecureLearnableEmbeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, position_ids: torch.Tensor | None = None,
                token_type_ids: torch.Tensor | None = None) -> SecureValue:
...
class SecureAdaptiveEmbedding(nn.Module):
    ...
    def __init__(self, config, protocol: Protocol, cutoffs: list = None):
        ...
        for i, (cutoff, next_cutoff) in enumerate(zip([0] + cutoffs[:-1], cutoffs, strict=False)):
    ...
    def forward(self, input_ids: torch.Tensor) -> SecureValue:
        ...
        for i, (cutoff, next_cutoff) in enumerate(zip([0] + self.cutoffs[:-1], self.cutoffs, strict=False)):
