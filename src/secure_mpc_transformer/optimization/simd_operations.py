...
try:
    import numba
    from numba import float32, float64, int32, int64, jit, vectorize
...
class VectorizedElementwise(SIMDOperation):
    ...
    def apply(self, data: np.ndarray | tuple[np.ndarray, np.ndarray]) -> np.ndarray:
    ...
    def _fallback_operation(self, data: np.ndarray | tuple[np.ndarray, np.ndarray]) -> np.ndarray:
...
class VectorizedReduction(SIMDOperation):
    ...
    def __init__(self, operation: str, axis: int | None = None):
        self.operation = operation
        self.axis = axis
...
class SIMDProcessor:
    ...
    def __init__(self, config: SIMDConfig | None = None):
