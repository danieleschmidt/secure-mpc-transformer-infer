...
class ShutdownComponent:
    ...
    async_shutdown_func: Callable[[], None] | None = None
    ...
    def _graceful_shutdown(self):
        ...
        for component in self.components:
            if time.time() - start_time > self.graceful_timeout:
                logger.warning("Graceful shutdown timeout exceeded, switching to forced shutdown")
                self._forced_shutdown()
                return
...
def wait_for_shutdown(timeout: float | None = None):
