...
class AuditRepository(BaseRepository[AuditLog]):
    ...
    async def get_by_event_type(self, event_type: AuditEventType,
                               limit: int | None = None) -> list[AuditLog]:
        ...
        if self.db_manager.database_type.value == "postgresql":
            ...
            async with self.db_manager.get_connection() as conn:
                async with conn.execute(query, (cutoff_time.isoformat(),)) as cursor:
                    ...
                    by_event_risk = [dict(zip(columns, row, strict=False)) for row in rows]
                ...
                async with conn.execute(total_query, (cutoff_time.isoformat(),)) as cursor:
                    ...
                    totals = dict(zip(total_columns, total_row, strict=False)) if total_row else {}
    ...
    async def get_recent_logs(self, hours: int = 24, limit: int | None = 100) -> list[AuditLog]:
