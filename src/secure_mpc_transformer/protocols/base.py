...
class SecureValue:
    ...
    def to(self, device: str | torch.device) -> "SecureValue":
    ...
    def __add__(self, other: "SecureValue") -> "SecureValue":
        ...
        return SecureValue(
            shares=[s1 + s2 for s1, s2 in zip(self.shares, other.shares, strict=False)],
            party_id=self.party_id,
            is_public=self.is_public and other.is_public
        )

    def __mul__(self, scalar: float | int | torch.Tensor) -> "SecureValue":
...
class Protocol(ABC):
    ...
    def __init__(self, party_id: int, num_parties: int, device: torch.device | None = None):
    ...
    def secure_linear(self, input_val: SecureValue, weight: SecureValue,
                     bias: SecureValue | None = None) -> SecureValue:
