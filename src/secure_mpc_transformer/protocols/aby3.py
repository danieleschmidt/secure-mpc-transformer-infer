...
class ABY3Protocol(Protocol):
    ...
    def __init__(self, party_id: int, num_parties: int = 3,
                 device: torch.device | None = None, ring_size: int = 2**64):
        if num_parties != 3:
    ...
    def secure_add(self, a: SecureValue, b: SecureValue) -> SecureValue:
        ...
        # Add shares locally
        result_shares = []
        for share_a, share_b in zip(a_arith.shares, b_arith.shares, strict=False):
    ...
    def _boolean_comparison_circuit(self, a_shares: list[torch.Tensor],
        ...
        result_shares = []
        for share_a, share_b in zip(a_shares, b_shares, strict=False):
